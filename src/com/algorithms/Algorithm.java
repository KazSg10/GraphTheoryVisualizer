...
public class Algorithm {
	...
	public void depthFirstTraversal(UnweightedGraph graph, Node visitedNode, List<Node> visited, Stack<Node> stack ){
		...
		/*
		 * If visited list and stack have not been created yet, then they are created.
		 * This is done in the first pass.
		 */
		if(visited == null) {
		...
		if(neighboursList != null) {
			for(Node neighbouringNode: neighboursList) {
				...
				//If the neighbouring node has not been visited yet, then the algorithm recurses with neighbouring node being visitedNode
		...
		//Popping the top node from the stack to backtrack
	...
	public void BreadthFirstTraversal(UnweightedGraph graph, Node sourceNode){
		...
		//Adding the source node to the queue at the start
		...
		while(!queue.isEmpty())
		{
			...
			//The top element in the queue is dequeued and is made the node to be visited
			...
			//Retrieving the list of neighbours of the visited node
			List<Node> neighboursList = graph.getNodeConnections(visitedNode);
			if(!(neighboursList == null)) {
				for(Node neighbouringNode: neighboursList) {
					c_graphOutputData.getSimulationSteps().add(new BFSSimulationSteps(null, 7, null, null, null));
					//If visited doesn't contain the neighbouring node, and queue doesn't contain the neighbouring node, then it is added to the queue
	...
	public void DijkstraShortestPathFinding(WeightedGraph graph, Node sourceNode){
		...
		//Creating a list of traversed nodes (nodes who have had all their paths, connecting itself and its neighbours, travelled along)
